                ...
                return userData.filter(dateis => dateis.Service_Start >= holdStart && dateis.Service_Start <= endDate).map((dateis, index) => {
                    return (
                        <tr key={dateis.ID}>
                            ...
                            <td><button style={{ height:"25px",width:"100%",borderRadius:"5px",'backgroundColor': dateis.Status === '0' ? '#66BB6A' : dateis.Status === '1' ? '#F44336' : dateis.Status === '2' ? 'orange' : dateis.Status === '3' ? 'yellow' : 'white' }}>{dateis.Status}</button></td>
